...
3. Optionally select a default domain from your Dub account.

## Usage

Once configured, a **Dub** panel will appear in the sidebar of any entry that belongs to a section with URLs.

- **Creating a short link** — enter a custom key in the sidebar field and save the entry. If left blank, no short link is created.
- **Updating a short link** — change the key in the sidebar and save. The existing Dub link is updated in place.
- **Deleting a short link** — the short link is automatically removed from Dub when an entry is deleted or its status changes from Live.
